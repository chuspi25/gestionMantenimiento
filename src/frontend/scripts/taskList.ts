import { Task, TaskFilters, TaskSortOptions } from './types.js';
...
export class TaskList {
    ...
    private currentSort: TaskSortOptions = { field: 'dueDate', direction: 'asc' };
    private currentView: 'grid' | 'list' = 'grid';
    private searchTerm: string = '';
    private isLoading: boolean = false;
    private error: string | null = null;
    private onViewTask?: (task: Task) => void;
    private onEditTask?: (task: Task) => void;
    private onUpdateTaskStatus?: (task: Task) => void;
    ...
    private setupEventListeners(): void {
        ...
        const filterAssigned = document.getElementById('filter-assigned') as HTMLSelectElement;
        
        filterType?.addEventListener('change', (e) => {
            const target = e.target as HTMLSelectElement;
            this.currentFilters.type = (target.value as any) || undefined;
            this.applyFiltersAndSort();
        });
        
        filterStatus?.addEventListener('change', (e) => {
            const target = e.target as HTMLSelectElement;
            this.currentFilters.status = (target.value as any) || undefined;
            this.applyFiltersAndSort();
        });
        
        filterPriority?.addEventListener('change', (e) => {
            const target = e.target as HTMLSelectElement;
            this.currentFilters.priority = (target.value as any) || undefined;
            this.applyFiltersAndSort();
        });
        ...
        const sortDirection = document.getElementById('sort-direction') as HTMLSelectElement;
        
        sortBy?.addEventListener('change', (e) => {
            const target = e.target as HTMLSelectElement;
            this.currentSort.field = target.value as keyof Task;
            this.applyFiltersAndSort();
        });
        
        sortDirection?.addEventListener('change', (e) => {
            const target = e.target as HTMLSelectElement;
            this.currentSort.direction = target.value as 'asc' | 'desc';
            this.applyFiltersAndSort();
        });
        ...
        // Vista
        const viewGridButton = document.getElementById('view-grid') as HTMLButtonElement;
        const viewListButton = document.getElementById('view-list') as HTMLButtonElement;
     ...
     * Esperar a que el token esté disponible
     */
    private async waitForToken(maxAttempts: number = 20): Promise<void> {
     ...
     */
    private viewTaskDetails(task: Task): void {
     ...
     */
    public getCurrentFilters(): TaskFilters {
