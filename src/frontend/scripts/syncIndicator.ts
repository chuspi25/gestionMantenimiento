...
export class SyncIndicator {
    ...
    private updateUI(): void {
        ...
        const manualSyncBtn = this.container.querySelector('#manual-sync-btn') as HTMLButtonElement;
        const retryBtn = this.container.querySelector('#retry-btn') as HTMLElement;

        if (statusIcon) statusIcon.innerHTML = this.getStatusIcon();
        if (statusText) statusText.textContent = this.getStatusText();
        if (connectionStatus) connectionStatus.textContent = this.getConnectionText();
        if (lastSync) lastSync.textContent = this.getLastSyncText();
        if (pendingActions) pendingActions.textContent = this.getPendingActionsText();
        if (localTasks) localTasks.textContent = this.getLocalTasksText();
    ...
    private toggleDetails(): void {
        const details = this.container.querySelector('#sync-details') as HTMLElement;
        const actions = this.container.querySelector('#sync-actions') as HTMLElement;
     ...
     */
    private getPendingActionsText(): string {
     ...
     */
    private getLocalTasksText(): string {
...
type ConnectionStatus = 'online' | 'offline' | 'syncing' | 'pending' | 'error';
