import { Task, TaskNote, TaskAttachment } from './types.js';
...
export class TaskDetail {
     ...
     */
    private setupTaskDetailEventListeners(): void {
     ...
     * Renderizar notas
     */
    private renderNotes(notes: TaskNote[]): string {
        if (notes.length === 0) {
            return `
     ...
     */
    private renderAttachments(attachments: TaskAttachment[]): string {
     ...
     */
    private showAddAttachmentModal(): void {
     ...
     */
    private async addNote(content: string): Promise<void> {
        if (!this.task) return;
     ...
     */
    private isStatusAfter(status1: string, status2: string): boolean {
     ...
     */
    private formatFileSize(_fileType: string): string {
