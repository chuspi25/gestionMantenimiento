...
export class ReportingInterface {
     ...
     */
    private getDefaultFilters(): ReportFilters {
     ...
     */
    private renderPriorityChart(): string {
        if (!this.reportData) return '';
     ...
     */
    private renderTypeChart(): string {
        if (!this.reportData) return '';
     ...
     */
    private renderUserProductivity(): string {
        if (!this.reportData) return '';
     ...
     */
    private populateUserSelects(): void {
        const assignedToSelect = document.getElementById('assigned-to-select') as HTMLSelectElement;
        const createdBySelect = document.getElementById('created-by-select') as HTMLSelectElement;
     ...
     */
    private getCheckedValues(groupId: string): string[] {
        const group = document.getElementById(groupId);
        if (!group) return [];

        const checkboxes = group.querySelectorAll('input[type="checkbox"]:checked') as NodeListOf<HTMLInputElement>;
    ...
    private clearFilters(): void {
        ...
        if (assignedToSelect) assignedToSelect.selectedIndex = 0;
        if (createdBySelect) createdBySelect.selectedIndex = 0;
    }
     ...
     * Exportar reporte
     */
    private async exportReport(format: 'pdf' | 'excel' | 'csv'): Promise<void> {
        if (!this.reportData) {
     ...
     */
    public getCurrentReportData(): ReportData | null {
     ...
     */
    public getCurrentFilters(): ReportFilters {
        return { ...this.filters };
    }
}