...
export class TaskProgress {
    ...
    private setupEventListeners(): void {
        ...
        fileInput?.addEventListener('change', () => {
            ...
            const uploadButton = uploadBtn as HTMLButtonElement;
    ...
    private async handleStatusUpdate(): Promise<void> {
        ...
        const newStatusSelect = document.getElementById('new-status') as HTMLSelectElement;
        const statusNoteTextarea = document.getElementById('status-note') as HTMLTextAreaElement;

        const newStatus = newStatusSelect.value as Task['status'];
    ...
    private async handleFileUpload(): Promise<void> {
        ...
        const fileInput = document.getElementById('file-input') as HTMLInputElement;
        const fileDescriptionInput = document.getElementById('file-description') as HTMLInputElement;
        ...
        try {
            ...
            const uploadButton = document.getElementById('upload-file-btn') as HTMLButtonElement;
            if (uploadButton) uploadButton.disabled = true;
            const fileDescriptionGroup = document.getElementById('file-description-group');
            if (fileDescriptionGroup) fileDescriptionGroup.style.display = 'none';
     ...
     */
    private async addNoteToTask(content: string): Promise<void> {
        const response = await fetch(`/api/tasks/${this.task!.id}/notes`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
    ...
    private renderRecentActivity(): string {
        if (!this.task) return '<p class="no-activity">No hay actividad reciente</p>';
     ...
     */
    private getStatusOptions(currentStatus: Task['status']): Array<{value: string, label: string}> {
        const allOptions = [
     ...
     */
    private getPriorityClass(priority: Task['priority']): string {
        const classes = {
            'low': 'priority-low',
     ...
     */
    private getStatusClass(status: Task['status']): string {
        const classes = {
