...
/**
 * Configuración de rutas y sus permisos requeridos
 */
...
export class RoleGuard {
   ...
   */
  navigateToRoute(routePath: string): boolean {
   ...
   */
  removeRouteChangeListener(listener: (route: string) => void): void {
   ...
   */
  canPerformAction(action: keyof RolePermissions, resourceOwnerId?: string): boolean {
    const permissions = this.getCurrentUserPermissions();
   ...
   */
  generateNavigation(): Array<{ route: RouteConfig; isActive: boolean }> {
   ...
   */
  shouldShowElement(permission: keyof RolePermissions, resourceOwnerId?: string): boolean {
   ...
   */
  getRestrictionMessage(permission: keyof RolePermissions): string {
    const user = this.authManager.getCurrentUser();
    if (!user) return 'Debes iniciar sesión para realizar esta acción';

    const messages: Record<keyof RolePermissions, string> = {
...
export class RoleGuardUI {
  ...
  constructor(roleGuard: RoleGuard) {
  ...
  applyAccessControl(): void {
    ...
    protectedElements.forEach(element => {
      const permission = element.getAttribute('data-permission') as keyof RolePermissions;
      ...
      if (permission && !this.roleGuard.shouldShowElement(permission, resourceOwnerId || undefined)) {
        ...
        if (showMessage) {
          this.showRestrictionMessage(element as HTMLElement, permission);
        }
      } else {
        // Mostrar elemento
   ...
   */
  private showRestrictionMessage(element: HTMLElement, permission: keyof RolePermissions): void {
   ...
   */
  generateNavigationMenu(containerId: string): void {
   ...
   */
  updateNavigation(containerId: string): void {
